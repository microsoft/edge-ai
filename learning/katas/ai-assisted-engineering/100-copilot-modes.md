---
title: 'Kata: 100 - GitHub Copilot Agents'
description: Learn GitHub Copilot agent mode mechanics including Ask, Edit, and Agent modes with practical workflows for effective mode selection and transitions
author: Edge AI Team
ms.date: 2025-01-24
...
common_pitfalls:
  - "Using Edit agent for exploratory questions - use Ask agent instead"
  - "Using Ask agent for multi-file changes - switch to Edit agent"
  - "Forgetting to provide context (@workspace or #-mentions) in Edit agent"
...
**You'll Learn**: GitHub Copilot agent mechanics including Ask, Edit, and Agent agents with practical workflows for effective agent selection and transitions.
...
**Real Challenge**: You're working on a feature that requires understanding existing code (Ask), making coordinated changes across files (Edit), and researching implementation patterns (Agent). Knowing which agent to use and how to transition between them is crucial for productivity.
**Your Task**: Practice activating and using each Copilot agent, understand their strengths, and develop intuition for agent selection based on task characteristics.
...
>
> ```text
> I'm working on GitHub Copilot Custom Agent kata and want interactive coaching with progress tracking.
> ```
...
### Task 2: Edit Agent - Coordinated Multi-file Exploration (12 minutes)

**What You'll Do**: Learn Edit agent for exploring coordination patterns across multiple files in real repository code using @workspace and #-mentions

<!-- AI_COACH: Edit agent is task-oriented for both exploration and execution. This task focuses on read-only exploration to understand patterns. Guide learners: @workspace for discovery, #-mentions for known files. Emphasize reviewing proposed changes without applying them to production code. -->

**Steps**:

1. **Understand** Edit agent fundamentals
   - [ ] **Key concept**: Edit agent coordinates exploration and changes across multiple files using context mechanisms
   - [ ] **Activation**: Select "Edit" from the agent selector dropdown in Chat panel (Ctrl+Alt+I / Cmd+Alt+I)
   ...
   - [ ] **Expected result**: Clear understanding of Edit agent's purpose and activation via agent selector

2. **Compare** Edit agent vs. Ask mode
   - [ ] **Ask mode**: Questions, conceptual exploration, explanations, conversational
   - [ ] **Edit agent**: Multi-file exploration with diffs, coordinated changes, @workspace/#-mentions context, task-oriented
   - [ ] **Transition signal**: When you say "I need to see how this works across files" or "I need to update this in multiple files"
   - [ ] **Expected result**: Understanding when to switch from Ask to Edit agent
...
3. **Discover** real repository files for exploration
   ...
   - [ ] **Expected result**: Familiarity with real infrastructure components for Edit agent practice
4. **Activate** Edit agent via agent selector
   - [ ] Open GitHub Copilot Chat: `Ctrl+Alt+I` / `Cmd+Alt+I`
   - [ ] Click the agent selector dropdown at the top of the chat panel
   - [ ] Select "Edit" agent from the dropdown
   - [ ] Verify the agent indicator shows "Edit"
   - [ ] **Expected result**: Chat panel in Edit agent mode, ready for multi-file editing

5. **Explore** coordination patterns with @workspace context (READ-ONLY)
   - [ ] In Chat panel (Edit agent), enter:
   ...
   - [ ] Wait for Edit agent to discover the files and show patterns
   - [ ] Review the common patterns identified (location, environment, resource_prefix)
   - [ ] **Pro tip**: Edit agent can discover and analyze patterns across many files
   - [ ] **DO NOT APPLY** any changes - this is exploration only
   - [ ] **Expected result**: Understanding how Edit agent discovers and analyzes patterns across multiple files

6. **Practice** using #-file mentions for specific targeting
   - [ ] In Chat panel (still in Edit agent), continue with new prompt:
   ...
   - [ ] **Expected result**: Comfortable using both @workspace and #-mentions for Edit agent context
...
1. **Understand** Agent Mode fundamentals
   ...
   - [ ] **Activation**: Select "Agent" mode in Copilot Agent selector dropdown
...
2. **Understand** mode selection decision matrix
   ...
   - [ ] **Task**: "Update Y in files A, B, C" → **Mode**: Edit agent → **Why**: Coordinated changes
   ...
   - [ ] **Task**: "Refactor across modules" → **Mode**: Edit agent → **Why**: Multi-file coordination
...
6. **Practice** mode transition workflow
   ...
   - [ ] **Switch to Edit agent**: Use @workspace or #-mentions to request pattern application
...
- [ ] Demonstrated effective use of Edit agent for multi-file exploration using @workspace and #-mentions on real repository files
...
- **Edit agent**: Dual purpose - exploration (propose changes to understand patterns) and execution (apply changes). Provide context via @workspace (discovery) or #-mentions (specific files).
- **Agent mode**: Best for research tasks requiring multiple sources or comprehensive analysis.
- **Mode transitions**: Research (Agent) → Understand (Ask) → Explore patterns (Edit agent) → Implement (Edit agent) is a common workflow.
- **Context strategies**: Use @workspace for discovery when file set unknown; use #-mentions for known file sets.
- **Exploration workflow**: Use Edit agent to propose changes on real code without applying them to learn coordination patterns safely.

### Troubleshooting

**Q: Edit agent doesn't show suggested changes**
**A**: Ensure you provided context via @workspace or #-mentions. Edit agent needs file context to discover and suggest changes.
...
[kata-200-basics]: /learning/katas/ai-assisted-engineering/200-copilot-edit-agent-basics.md
