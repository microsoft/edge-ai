# Tests for Full Multi Node Cluster Blueprint

This directory contains comprehensive integration and contract tests for the [Full Multi Node Cluster](../README.md) blueprint supporting both Terraform and Bicep deployments.
...
|--------------------------|-------------------------------------------|-------------------------------------------|
| **Test Function**        | `TestTerraformFullMultiNodeClusterDeploy` | `TestBicepFullMultiNodeClusterDeploy`     |
...
go test -v -run TestBicepFullMultiNodeClusterDeploy -timeout 30m
...
go test -v -run TestTerraformFullMultiNodeClusterDeploy/Validate -timeout 30m

# Clean up when done
export CLEANUP_RESOURCES=true
go test -v -run TestTerraformFullMultiNodeClusterDeploy -timeout 15m
```
