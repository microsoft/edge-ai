...
| cloud\_notification | ../../../src/000-cloud/045-notification/terraform | n/a |
...
| acr\_export\_policy\_enabled | Whether to allow container image export from the ACR. Requires acr\_public\_network\_access\_enabled to be true when enabled | `bool` | `false` | no |
...
| alert\_eventhub\_consumer\_group | Consumer group for the alert notification Function App Event Hub trigger. Otherwise, '$Default' | `string` | `"$Default"` | no |
| alert\_eventhub\_name | Name of the Event Hub for inference alerts. Otherwise, 'evh-{resource\_prefix}-alerts-{environment}-{instance}' | `string` | `null` | no |
...
| arc\_machine\_name | Exact name of a single existing Arc-enabled machine to target; honored when arc\_machine\_count is 1 in place of the name-prefix model | `string` | `null` | no |
...
| closure\_message\_template | HTML message body for session-closure Teams notifications. Supports Logic App expression syntax for dynamic fields | `string` | `"<p>Session closed for event.</p>"` | no |
...
| function\_app\_settings | Application settings for the Function App deployed by the messaging component | `map(string)` | `{}` | no |
| host\_machine\_count | Number of edge host virtual machines to create for the multi-node cluster | `number` | `1` | no |
...
| notification\_event\_schema | JSON schema object for parsing Event Hub events in the Logic App Parse\_Event action | `any` | `{}` | no |
| notification\_message\_template | HTML template for new-event Teams notifications. Supports Terraform template variable: close\_session\_url. Supports Logic App expression syntax for dynamic event fields | `string` | `"<p>New alert event detected.</p>"` | no |
| notification\_partition\_key\_field | Caller's event schema field name to use as the Table Storage partition key for session-state deduplication lookups (e.g. "event\_id", "asset\_id"). Must be set by the scenario tfvars. | `string` | `"event_id"` | no |
...
| postgresql\_admin\_password\_wo\_version | Version used to trigger updates for write-only PostgreSQL administrator password arguments. Increment this value when rotating the administrator password. | `number` | `1` | no |
...
| should\_deploy\_aio | Whether to deploy Azure IoT Operations and its dependent edge components (assets, edge messaging). When false, deploys Arc-connected cluster with extensions and observability only | `bool` | `true` | no |
...
| should\_deploy\_notification | Whether to deploy the 045-notification Logic App for alert deduplication and Teams posting | `bool` | `false` | no |
...
| should\_enable\_key\_vault\_purge\_protection | Whether to enable purge protection for the Key Vault. Enable for production to prevent accidental or malicious secret deletion | `bool` | `false` | no |
...
| should\_enable\_observability\_private\_endpoints | Override for Azure Monitor (observability) private endpoints. Defaults to should\_enable\_private\_endpoints when null. Set false for Arc edge clusters, where the managed Prometheus metrics addon cannot fetch its config or ingest over private link and would otherwise leave dashboards empty | `bool` | `null` | no |
...
| teams\_group\_id | Microsoft 365 Group ID (Team ID) for posting to a Teams channel. Required when teams\_post\_location is 'Channel' | `string` | `null` | no |
| teams\_post\_location | Teams posting location type for the notification message: 'Channel' for a Teams channel or 'Group chat' for a group chat | `string` | `"Channel"` | no |
| teams\_recipient\_id | Teams chat or channel thread ID for posting event notifications | `string` | `null` | no |
