...
variable "arc_machine_name" {
  type        = string
  description = "Name of the existing Azure Arc-enabled machine that will host the K3s cluster"

  validation {
    condition     = length(var.arc_machine_name) > 0
    error_message = "arc_machine_name must not be empty."
  }
}

variable "arc_machine_resource_group_name" {
  type        = string
  description = "Resource group name containing the Arc-enabled machine. When null, defaults to the blueprint cloud resource group"
  default     = null
}

variable "cluster_server_host_machine_username" {
  type        = string
  description = "Linux user on the Arc-enabled machine that owns the kubeconfig (typically the user that ran the Arc onboarding)"
}
