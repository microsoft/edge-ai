...
module "cloud_kubernetes" {
  ...
  aks_command_invoke_configurations = merge(
    var.should_install_robotics_charts ? {
      "install-robotics-charts" = {
        command     = "./install-robotics-charts.sh"
        folder_path = "${path.module}/scripts"
      }
    } : {},
    var.should_install_azureml_charts ? {
      "install-azureml-charts" = {
        command     = "./install-azureml-charts.sh"
        folder_path = "${path.module}/scripts"
      }
    } : {}
  )
...
module "cloud_azureml" {
  ...
  default_outbound_access_enabled         = local.default_outbound_access_enabled
  should_associate_network_security_group = var.should_create_networking
  should_enable_nat_gateway               = alltrue([var.should_enable_managed_outbound_access, var.should_create_networking])
  ...
  should_assign_ml_workload_identity_roles = alltrue([var.should_create_ml_workload_identity, var.should_create_security_identity])
