...
# Exclude tests and .terraform directories. Remove duplicates with `sort -u`.
find "$script_dir/../src" "$script_dir/../blueprints" "$script_dir/../deploy/azdo" \
  -type d \( -name "tests" -o -name ".terraform" \) -prune -false -o \
...
find "$script_dir/../src" "$script_dir/../blueprints" "$script_dir/../deploy/azdo" \
  -type d \( -name "tests" -o -name ".terraform" \) -prune -false -o \
  \( -path "*/terraform/README.md" -o -path "*/terraform/modules/*/README.md" -o -path "*/deploy/azdo/README.md" -o -path "*/deploy/azdo/modules/*/README.md" \) -type f -print0 \
