...
helm upgrade --install aio-observability open-telemetry/opentelemetry-collector \
  -f "$TF_MODULE_PATH/yaml/otel_collector/otel-collector-values.yaml" \
  --namespace "$TF_AIO_NAMESPACE" \
  --wait \
  --kubeconfig "$kube_config_file"

# Creating ConfigMap for Azure Monitor (only used if Azure Monitor extensions is installed)
until envsubst <"$TF_MODULE_PATH/yaml/otel_collector/ama-metrics-prometheus-config.yaml" | kubectl apply -f - --kubeconfig "$kube_config_file"; do
