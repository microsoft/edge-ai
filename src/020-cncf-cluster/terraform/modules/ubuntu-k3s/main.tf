...
locals {
  ...
  common_env_var = {
    ...
    ARC_SP_CLIENT_ID     = coalesce(var.arc_onboarding_sp_client_id, "$${ARC_SP_CLIENT_ID}")
    ARC_SP_SECRET        = coalesce(var.arc_onboarding_sp_client_secret, "$${ARC_SP_SECRET}")
  ...
  script_server_rendered = join("###\n", concat([local.script_server_parameters], slice(local.script_file, 1, length(local.script_file))))

  ## Agent
  # Apply Terraform templated variables to the parameters part of the script file.
  script_node_parameters = templatestring(local.script_file[0], merge(local.common_env_var, local.node_env_var))
  # Join the script back together with the rendered parameters along with the rest of the file.
  script_node_rendered = join("###\n", concat([local.script_node_parameters], slice(local.script_file, 1, length(local.script_file))))
}
