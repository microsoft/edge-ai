...
data "external" "public_key" {
  program = ["bash", "-c", "python3 -m pip install pycryptodome 1>/dev/null && python3 ${path.module}/../../../scripts/convert_to_pub_key.py \"${tls_private_key.arc_key.private_key_pem}\""]
}
