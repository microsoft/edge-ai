...
locals {
  ...
  location              = "centralus"
  subnet_name           = "snet-${local.resource_prefix}"
  subnet_id             = "${local.subscription_id_part}/resourceGroups/${local.resource_group_name}/providers/Microsoft.Network/virtualNetworks/vnet-${local.resource_prefix}/subnets/${local.subnet_name}"
}
