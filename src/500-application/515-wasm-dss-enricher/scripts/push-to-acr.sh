...
# Push dss-enricher WASM module and graph definition to ACR
# Usage: ./push-to-acr.sh <acr_name> [app_path]

ACR_NAME="${1:?ACR name required}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_DIR="${2:-${SCRIPT_DIR}/..}"
OPERATOR_DIR="${APP_DIR}/operators/dss-enricher"
...
WASM_FILE="${OPERATOR_DIR}/target/wasm32-wasip2/release/dss_enricher.wasm"
if [[ ! -f "${WASM_FILE}" ]]; then
  echo "WASM module not found. Run build-wasm.sh first."
  exit 1
fi

echo "Pushing dss-enricher module v${VERSION}"
oras push \
  "${ACR_NAME}.azurecr.io/dss-enricher:${VERSION}" \
  --artifact-type application/vnd.module.wasm.content.layer.v1+wasm \
  "${WASM_FILE}:application/wasm" \
  --disable-path-validation

GRAPH_FILE="${APP_DIR}/resources/graphs/graph-dss-enricher.yaml"
if [[ -f "${GRAPH_FILE}" ]]; then
  ...
  oras push \
    "${ACR_NAME}.azurecr.io/dss-enricher-graph:${VERSION}" \
    --config \
