...
- **Namespaced Devices**: Kubernetes resources defining REST endpoint configurations
- **Namespaced Assets**: Resources configuring data sources and destinations
...
2. **Test mock REST endpoints:**
   ...
   curl -u device_user:device_pass_123 http://localhost:8082/api/device/status
   ```
...
DEVICE_PASSWORD=device_pass_123
...
curl -u device_user:device_pass_123 http://localhost:8082/api/device/status | jq .
```
